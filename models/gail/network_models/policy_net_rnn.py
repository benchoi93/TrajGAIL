...
from network_models.policy_net_pytorch import Policy_net as Policy_vanilla
from network_models.policy_net_pytorch import Value_net as Value_vanilla
...
class StateSeqEmb(nn.Module):
    ...
    def forward(self, state_seq, seq_len):
        ...
        packed_input = pack_padded_sequence(x_emb, seq_len.tolist(), batch_first =True)
...
class Value_infoGAIL(Value_vanilla):
    def __init__(self,state_dim,action_dim ,encode_dim, hidden:int, num_layers = 3):
        ...
        self.encode_fc1 = nn.Linear(in_features = self.encode_dim , out_features=self.hidden)
        self.encode_fc2 = nn.Linear(in_features = self.hidden , out_features=self.hidden)
    
        self.fc1 = nn.Linear(in_features = self.hidden*2+self.action_dim , out_features=self.hidden*2)
        self.fc2 = nn.Linear(in_features = self.hidden*2 , out_features=self.hidden*2)
        self.fc3 = nn.Linear(in_features = self.hidden*2 , out_features=self.hidden)
...
class Policy_infoGAIL(Policy_vanilla):
    def __init__(self, state_dim, action_dim ,hidden:int,origins,start_code, encode_dim,disttype = "categorical", num_layers = 3):
        super(Policy_infoGAIL, self).__init__(state_dim, action_dim, hidden, origins, start_code, disttype)
        """
        :param name: string
        :param env: gym env
        """
        ...
        self.fc1 = nn.Linear(in_features = self.hidden*2 , out_features=self.hidden*2)
        self.fc2 = nn.Linear(in_features = self.hidden*2 , out_features=self.hidden*2)
        self.fc3 = nn.Linear(in_features = self.hidden*2 , out_features=self.hidden)

        self.pfc1 = nn.Linear(in_features = self.hidden , out_features=self.hidden)
        self.pfc2 = nn.Linear(in_features = self.hidden , out_features=self.hidden)
        self.pfc3 = nn.Linear(in_features = self.hidden , out_features=self.hidden)
    ...
    def forward(self, state_seq, seq_len, encode):
        ...
        action_dist = torch.distributions.relaxed_categorical.RelaxedOneHotCategorical(temperature= 1,probs = prob)
        # action_dist = torch.distributions.Categorical(prob)
        return action_dist
    ...
    def pretrain_forward(self, state_seq, seq_len):
        ...
        prob = torch.zeros(size = (temp_stateseq.size(0) , self.prob_dim) , device = h.device)
        prob[origins_idx] = origins_prob
        prob[~origins_idx,:self.action_dim] = others_prob
...
class Policy_net(Policy_vanilla):
    def __init__(self, state_dim, action_dim ,hidden:int,origins,start_code, disttype = "categorical", num_layers = 3):
        super(Policy_net, self).__init__(state_dim, action_dim, hidden, origins, start_code, disttype)
        """
        :param name: string
        :param env: gym env
        """
