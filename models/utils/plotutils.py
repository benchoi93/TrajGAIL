...
def plot_summary(GAIL, exp_trajs , learner_observations, keep_unknown=True):
    ...
    unknown_cnt = sum(pd_route_dist.loc[pd_route_dist['expert']== 0]['gail'])
    unknown= [-1.0, 0.0 , unknown_cnt]
    unknown = pd.DataFrame(unknown).T
    unknown.columns = pd_route_dist.columns
    pd_route_dist = pd_route_dist.loc[pd_route_dist['expert']!= 0]

    pd_route_dist = pd.concat([pd_route_dist, unknown])
    ...
    gail_cnt = []
    for r0 in routes:
        r0_np = np.ones((1,unq.shape[1]),dtype=np.int64) * -1
        r0 = list(map(int,r0.split("-")))

        slice0 = min(len(r0) , unq.shape[1])
        r0_np[0,:slice0] = r0[:slice0]

        find_r0  = np.where(np.all(unq == r0_np , axis =1))[0]
        if find_r0.shape[0] > 0:
            gail_cnt.append(cnt[find_r0[0]])
        else:
            gail_cnt.append(0)
    unknown_cnt = cnt.sum() - sum(gail_cnt)
