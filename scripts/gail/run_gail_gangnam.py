...
import torch
from network_models.policy_net_rnn import Policy_net,Value_net,StateSeqEmb
from network_models.discriminator_wgail import Discriminator as Discriminator_wgail
from network_models.discriminator_rnn import Discriminator as Discriminator_rnn
from network_models.infoq_rnn import infoQ_RNN
...
def argparser():
    ...
    parser.add_argument('-ud','--num-discrim-update', default=int(2) , type = int)
    parser.add_argument('-ug','--num-gen-update', default=int(6) , type = int)    
    parser.add_argument('-lr','--learning-rate' , default=float(5e-5), type = float)
    ...
    parser.add_argument('--data' , default = "data/gangnam_expert.pkl", type = str)
...
def main(args):
    ...
    env = shortestpath.ShortestPath("data/gangnam_Network.txt",origins,destinations)
    # exp_trajs = env.import_demonstrations(args.data)    
    exp_trajs = env.import_demonstrations(args.data)    
    # print(len(exp_trajs))
    overlengths= np.array(list(map(len,exp_trajs)) ) > args.max_length
    if sum(overlengths) > 0:
        exp_trajs = np.array(exp_trajs)[~overlengths]
        exp_trajs = exp_trajs.tolist()
    # print(len(exp_trajs))
    ...
    find_state = lambda x: env.states.index(x) if x != -1 else pad_idx
    ...
    value  = Value_net(state_dim,origins.shape[0],hidden=args.hidden, num_layers = args.num_layers)
    if args.wasser:
        D   = Discriminator_wgail(state_dim,origins.shape[0],hidden=args.hidden, disttype = "categorical", num_layers = args.num_layers)        
    else:
        D   = Discriminator_rnn(state_dim,origins.shape[0],hidden=args.hidden, disttype = "categorical", num_layers = args.num_layers)
    ...
    if args.wasser:
        ...
        GAILRNN.discrim_opt = torch.optim.RMSprop(GAILRNN.Discrim.parameters(),lr = GAILRNN.lr,eps=GAILRNN.eps)
    ...
    for _ in range(args.iteration):
        ...
        learner_observations, learner_actions, learner_len, learner_rewards= \
            GAILRNN.unroll_trajectory2(num_trajs = args.n_episode , max_length = args.max_length)
        ...
        GAILRNN.summary.add_scalar('GenTrajs/reward', avg_reward ,GAILRNN.summary_cnt )            
        GAILRNN.summary.add_scalar('GenTrajs/step_reward', avg_ind_reward ,GAILRNN.summary_cnt )            
        GAILRNN.summary.add_scalar('GenTrajs/length', avg_len ,GAILRNN.summary_cnt )            
        plot_summary(GAILRNN, exp_trajs , learner_observations, keep_unknown=False)       
        ...
        GAILRNN.summary.add_scalar('GenTrajs/elapsed_time', elapsed ,GAILRNN.summary_cnt )            
        print("Total Reward = {:.2f} / Ind. Reward = {:.5f} / Length = {:.2f} / ElapsedTime = {:.2f}".format( avg_reward  ,avg_ind_reward , avg_len, elapsed) )
